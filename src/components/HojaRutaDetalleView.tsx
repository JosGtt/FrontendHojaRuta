...
const HojaRutaDetalleView: React.FC<HojaRutaDetalleViewProps> = ({ hoja, onBack }) => {
  ...
  const guardarEdicionSeccion = async (seccionData: any) => {
    ...
    try {
      ...
      const response = await axios.put(`${API_ENDPOINTS.HOJAS_RUTA}/${hojaCompleta.id}`, updateData, {
        headers: { 
  ...
  const guardarEdicionCompleta = async (formData: any) => {
    ...
    try {
      ...
      // Enviar actualización al backend
      const response = await axios.put(`${API_ENDPOINTS.HOJAS_RUTA}/${hojaCompleta.id}`, formData, {
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchHojaCompleta = async () => {
    ...
    try {
      ...
      console.log('🔍 Obteniendo detalles de hoja de ruta:', {
        hojaId: hoja.id,
        url: `${API_ENDPOINTS.HOJAS_RUTA}/${hoja.id}`,
        hasToken: !!token
      });
  ...
  const cambiarEstado = async (nuevoEstado: string) => {
    ...
    try {
      ...
      console.log('🔄 Preparando solicitud:', {
        endpoint: `${API_ENDPOINTS.HOJAS_RUTA}/${hojaCompleta.id}/estado`,
  ...
  const cambiarUbicacion = async (nuevaUbicacion: string, responsable: string) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_ENDPOINTS.HOJAS_RUTA}/${hojaCompleta.id}/ubicacion`,
        { 
